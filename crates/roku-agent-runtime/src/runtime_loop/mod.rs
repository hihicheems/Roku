...
pub use compact::{
	CompactConfig, MID_WATER_TRIGGER_RATIO, MidCompactOutcome, PER_TURN_TOOL_BUDGET_TOKENS,
	PerModelCalibration, PromptTokenEstimate, compact_history, compact_history_with_llm,
	compact_messages, compact_messages_with_structured_summary, estimate_context_tokens,
	estimate_prompt_pressure, estimate_prompt_tokens_calibrated, estimate_turn_tool_tokens,
	hash_message_prefix, hash_system_prompt_text, hash_tool_schema_bytes, mid_compact_messages,
...
// validation helper. `microcompact_old_tool_results` and
// `MICROCOMPACT_RETAIN_RECENT` join this allow-list because the runtime
// no longer invokes the pre-flight rewrite; they remain available to the
// time-gated path that consumes them once the prompt cache has expired.
#[allow(unused_imports)]
pub use compact::{
	CommittedBaseline, MAX_DROP_OLDEST_RETRIES, MICROCOMPACT_PLACEHOLDER,
	MICROCOMPACT_RETAIN_RECENT, STRUCTURED_SUMMARY_SECTIONS, StructuredCompactError,
	StructuredCompactOutcome, microcompact_old_tool_results, validate_structured_summary,
};
