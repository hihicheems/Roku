...
SQLITE_MEMORY_PATH_ENV='ROKU_RUNTIME__MEMORY__BACKENDS__SQLITE__PATH'
SQLITE_MEMORY_PATH_LEGACY_ENV='ROKU_SQLITE_PATH'
SQLITE_MEMORY_DEFAULT_PATH='.roku/state/control-plane.db'

readonly SQLITE_MEMORY_PATH_ENV SQLITE_MEMORY_PATH_LEGACY_ENV SQLITE_MEMORY_DEFAULT_PATH
...
component_detail() {
	case "${1:-}" in
	roku-memory)
		printf '%s\n' 'provider-neutral memory/session/control-plane ownership'
		;;
	roku-plugin-memory-sqlite)
		printf '%s\n' 'sqlite adapter for short-term, session, pending-loop, and control-plane persistence'
		;;
	roku-runtime-service)
	...
	roku-artifact-store)
		...
		;;
	roku-experiment-registry)
		printf '%s\n' 'file-backed experiment runs embedded via roku-runtime-service'
		;;
	roku-observability)
...
sqlite_memory_adapter_detail() {
	local path source
	if [[ -n "${!SQLITE_MEMORY_PATH_ENV:-}" ]]; then
		path="${!SQLITE_MEMORY_PATH_ENV}"
		source="${SQLITE_MEMORY_PATH_ENV}"
	elif [[ -n "${!SQLITE_MEMORY_PATH_LEGACY_ENV:-}" ]]; then
		path="${!SQLITE_MEMORY_PATH_LEGACY_ENV}"
		source="${SQLITE_MEMORY_PATH_LEGACY_ENV} (legacy alias)"
	else
		path="$SQLITE_MEMORY_DEFAULT_PATH"
		source='typed sqlite config default'
	fi
